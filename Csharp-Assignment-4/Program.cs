...
public class Program
{
    ...
    public void Task1()
    {
        ...
        var sortedSalaries = totalSalaries.OrderBy(x => x.TotalSalary);
    ...
    public void Task2()
    {
        var secondOldestEmployee = employeeList.OrderByDescending(x => x.Age).Skip(1).First();

        var totalMonthlySalary = salaryList.Where(s => s.EmployeeID == secondOldestEmployee.EmployeeID && s.Type == SalaryType.Monthly).Sum(s => s.Amount);

        Console.WriteLine($"Task 2: Details of the 2nd oldest employee ({secondOldestEmployee.EmployeeFirstName} {secondOldestEmployee.EmployeeLastName}):");
        Console.WriteLine($"Age: {secondOldestEmployee.Age}, Total Monthly Salary: {totalMonthlySalary}");
        Console.WriteLine();
    }
